...
st.write("To upload csv and excel files, cleaning data, convert file formates and visualisation.")
...
if files :
    for file in files:
      ...
      if st.checkbox(f"Remove Duplicates - {file.name}"): 
        ...
        formate_choise = st.radio(f"convert {file.name} to:" ,["csv", "Excel"], key=file.name)

        if st.button(f"Download {file.name} as {formate_choise}"):
          output = BytesIO()    
          if formate_choise == "csv":
           ...
           new_name = file.name.replace(ext, "xlxs")
